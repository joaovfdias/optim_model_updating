...
def run_trial(pop, gen, noise, MAC=None):
    """
        - noise = se tem ruido ou n
        - MAC = 1 se considerar / 0 se não considerar
    """
    parameters =    [
    ...
    input_dir = os.path.join(os.getcwd(), r'laje\input')
    base_script_filename = "script_laje.txt"
    base_freq_filename = f"out_base_freq_laje{noise}.txt" if (noise != 0) else "out_base_freq_laje.txt"
    base_modes_filename = f"out_base_modes_laje{noise}.txt" if noise else "out_base_modes_laje.txt"
    ...
    def fitness_function(param):
        ...
        paired_comp_freq, paired_comp_modes, mac_error_sum = SpecialFun.pair_modes_mac(comp_freq, comp_modes,
                                                                                       ansys.base_modes)  # adicionada etapa de pareamento, já retorna a soma dos MAC
    ...
    log_title = f"laje_GA_freq_MAC_N{noise}" if MAC else f"laje_GA_freq_N{noise}" # alterar nome do arquivo gerado, se quiser
